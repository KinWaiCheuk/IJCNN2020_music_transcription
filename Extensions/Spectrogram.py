...
class MelSpectrogram_f_range(torch.nn.Module):
    def __init__(self, sr=22050, n_fft=2048, n_mels=128, hop_length=512, window='hann', center=True, pad_mode='reflect', low=50, high=6000, freq_bins=None, norm=1):
        super(MelSpectrogram_f_range, self).__init__()
        self.stride = hop_length
        self.center = center
        self.pad_mode = pad_mode
        self.n_fft = n_fft
        ...
        # Create filter windows for stft
        wsin, wcos, self.bins2freq = create_fourier_kernals(n_fft, freq_bins=freq_bins, window=window, freq_scale='linear', sr=sr, low=low, high=high)
        self.wsin = torch.tensor(wsin, dtype=torch.float)
        self.wcos = torch.tensor(wcos, dtype=torch.float)
        
        # Creating kenral for mel spectrogram
        mel_basis = mel(sr, freq_bins, n_mels, low, high, htk=False, norm=norm)
